...
test("探索流首屏动态媒体保持高优先级加载，作者头像不走懒加载", async ({ page, request }) => {
  ...
  await expect(mediaImage).toHaveAttribute("width", "720");
  await expect(mediaImage).toHaveAttribute("height", "540");

  const mediaShell = postCard.locator(".timeline-media-card.image-shell").first();
  await expect(mediaShell).toBeVisible();

  const authorAvatar = postCard.locator(".timeline-author img.avatar-image").first();
  await expect(authorAvatar).toBeVisible();
  await expect(authorAvatar).toHaveAttribute("width", "160");
  await expect(authorAvatar).toHaveAttribute("height", "160");
  await expect(authorAvatar).not.toHaveAttribute("loading", "lazy");
  await expect(postCard.locator(".timeline-author .image-shell--avatar").first()).toBeVisible();

  await mediaShell.evaluate((shell) => {
    const previousImage = shell.querySelector("img.timeline-media-image");
    const failedImage = document.createElement("img");
    failedImage.className = previousImage?.className || "timeline-media-image";
    failedImage.alt = previousImage?.alt || "动态图片";
    failedImage.setAttribute("width", "720");
    failedImage.setAttribute("height", "540");
    failedImage.setAttribute("decoding", "async");
    shell.classList.remove("is-loaded", "is-failed");
    delete shell.dataset.imageHydrated;
    delete shell.dataset.imageObserved;
    previousImage?.replaceWith(failedImage);
    window.hydrateAsyncImages?.(shell.parentElement || document);
  });
  await expect(mediaShell).toHaveClass(/is-failed/);
  await expect(postCard.locator("img.timeline-media-image").first()).toHaveCSS("opacity", "0");
});

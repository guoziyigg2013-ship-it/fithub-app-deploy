...
- `npm run check:trial-write` 会额外跑线上写入闭环，验证注册、关注、动态、私信和预约这条真实写路径，适合每次大版本发布后执行。
...
- 用户协议、隐私政策、账号注销说明。
- Web/PWA 端已在 `我的 > 账户 > 协议与安全` 增加协议、隐私、权限说明和账号注销申请入口；小程序侧保留 `pages/legal` 与 `我的` 页入口。
- 举报入口、拉黑入口。
- 文本内容安全检测。

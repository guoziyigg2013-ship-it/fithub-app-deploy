...
- 支持对风险动态执行“隐藏动态/下架归档/恢复动态”。隐藏或下架后用户端主页、探索、收藏和通知不再展示该动态，也不能继续点赞、收藏或评论；后台仍保留审计记录，误处理时可恢复。
...
- `hidden`：隐藏动态，适合疑似违规或待复核内容。
- `deleted`：软下架归档，适合已确认不适合展示的内容。
- `active`：恢复动态。
...
- 举报对象扩展到评论、私信和个人主页入口。
- 当前默认使用软下架归档，不直接物理删除用户数据，便于申诉恢复和运营审计；如后续遇到强合规删除要求，再单独增加硬删除审批链路。
